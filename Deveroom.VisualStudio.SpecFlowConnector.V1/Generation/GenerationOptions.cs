...
namespace Deveroom.VisualStudio.SpecFlowConnector.Generation
{
    ...
    public class GenerationOptions
    {
        ...
        public static GenerationOptions Parse(string[] args)
        {
            ...
            var projectFolder = Path.GetFullPath(args[4]).TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar);
            return new GenerationOptions
            {
                FeatureFilePath = Path.GetFullPath(Path.Combine(projectFolder, args[0])),
                ConfigFilePath = string.IsNullOrWhiteSpace(args[1]) ? null : Path.GetFullPath(Path.Combine(projectFolder, args[1])),
