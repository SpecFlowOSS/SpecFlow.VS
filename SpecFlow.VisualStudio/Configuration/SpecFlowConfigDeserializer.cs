...
public class SpecFlowConfigDeserializer : IConfigDeserializer<DeveroomConfiguration>
{
    ...
    public void Populate(string jsonString, DeveroomConfiguration config)
    {
        ...
        if (specFlowJsonConfiguration.Language != null &&
            specFlowJsonConfiguration.Language.TryGetValue("feature", out var featureLanguage))
            config.DefaultFeatureLanguage = featureLanguage;
        if (specFlowJsonConfiguration.BindingCulture != null &&
            specFlowJsonConfiguration.BindingCulture.TryGetValue("name", out var bindingCulture))
            config.ConfiguredBindingCulture = bindingCulture;
    }

    private class SpecFlowJsonConfiguration
    {
