...
namespace SpecFlow.VisualStudio.Editor.Commands.Infrastructure
{
    ...
    public class DeveroomFeatureEditorCommandBroker : DeveroomEditorCommandBroker<IDeveroomFeatureEditorCommand>
    {
        ...
        public DeveroomFeatureEditorCommandBroker(IVsEditorAdaptersFactoryService adaptersFactory, [ImportMany] IEnumerable<IDeveroomFeatureEditorCommand> commands)
    ...
    {
        [ImportingConstructor]
        public DeveroomCodeEditorCommandBroker(IVsEditorAdaptersFactoryService adaptersFactory, [ImportMany] IEnumerable<IDeveroomCodeEditorCommand> commands)
            : base(adaptersFactory, commands)
        {
        }
    }

    public abstract class DeveroomEditorCommandBroker<TCommand> : IVsTextViewCreationListener where TCommand: IDeveroomEditorCommand
    {
        ...
        protected DeveroomEditorCommandBroker(IVsEditorAdaptersFactoryService adaptersFactory, [ImportMany] IEnumerable<TCommand> commands)
        {
            _adaptersFactory = adaptersFactory;
            _commands = commands.ToList();
            _editorCommandRegistry = new Lazy<Dictionary<DeveroomEditorCommandTargetKey, IDeveroomEditorCommand[]>>(BuildRegistry, LazyThreadSafetyMode.ExecutionAndPublication);
        }

        #region Command Filter
        private class EditorCommandsFilter : IOleCommandTarget
        {
            public EditorCommandsFilter(IWpfTextView textView, Dictionary<DeveroomEditorCommandTargetKey, IDeveroomEditorCommand[]> commandRegistry)
            {
                TextView = textView;
            ...
            private Dictionary<DeveroomEditorCommandTargetKey, IDeveroomEditorCommand[]> CommandRegistry { get; set; }
        ...
        private Dictionary<DeveroomEditorCommandTargetKey, IDeveroomEditorCommand[]> BuildRegistry()
        {
            ...
            {
                list.AddRange(editorCommand.Targets.Select(target => new KeyValuePair<DeveroomEditorCommandTargetKey, IDeveroomEditorCommand>(new DeveroomEditorCommandTargetKey(target.CommandGroup, target.CommandId), editorCommand)));
            }
