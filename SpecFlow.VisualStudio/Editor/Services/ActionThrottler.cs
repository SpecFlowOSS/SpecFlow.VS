...
{
    public IActionThrottler Build(Action action) => new ActionThrottler(action);
}
...
public class ActionThrottler : IActionThrottler
{
    ...
    private readonly Stopwatch _changeFrequencyMeasurer = new();
    ...
    {
        _dispatcherTimer.Stop();
        var typingStart = !_changeFrequencyMeasurer.IsRunning ||
                          _changeFrequencyMeasurer.ElapsedMilliseconds >= _typingStartDetectionMilliseconds;
