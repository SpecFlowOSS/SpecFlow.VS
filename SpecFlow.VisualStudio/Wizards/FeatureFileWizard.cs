        ...
        if (projectSettings.IsSpecFlowProject)
        {
            if (projectSettings.DesignTimeFeatureFileGenerationEnabled)
                ...
                wizardRunParameters.ProjectScope.IdeScope.Actions.ShowProblem(
                    $"In order to be able to run the SpecFlow scenarios as tests, you need to install the '{SpecFlowPackageDetector.SpecFlowToolsMsBuildGenerationPackageName}' NuGet package to the project.");
