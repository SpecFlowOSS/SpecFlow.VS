using System;

namespace Deveroom.VisualStudio.ProjectSystem.Actions
{
    public class QuestionDescription
    {
        public string Title { get; }
        public string Description { get; }
        public bool IncludeCancel { get; }
        public Action<QuestionDescription> YesCommand { get; }
        public Action<QuestionDescription> NoCommand { get; }

        public QuestionDescription(string title, string description, Action<QuestionDescription> yesCommand, Action<QuestionDescription> noCommand = null, bool includeCancel = false)
        {
            Description = description;
            YesCommand = yesCommand;
            NoCommand = noCommand;
            IncludeCancel = includeCancel;
            Title = title;
        }
    }
}